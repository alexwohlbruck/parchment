...
* 3D trees no longer shatter into a flickering patchwork looking straight down. The plan view pushes the camera far enough back to fake a flat-on projection, and at that distance the depth buffer could not tell the front of a crown from its back — so the two fought over every pixel, and since the back of a leaf is lit from the opposite side the tree broke into light and dark wedges that crawled as the camera moved. This is the same loss of depth precision that used to shatter the buildings, arriving by a different route
