...
// Default layers population
export async function populateDefaultLayers(userId: string) {
  return await db.transaction(async (tx) => {
    // Check if user already has any layers or groups
    ...
    // If user already has layers or groups, don't create defaults
    if (existingLayers.length > 0 || existingGroups.length > 0) {
      return
    }
    ...
    const groups = await Promise.all(
      defaultLayerGroups.map(async (group) => {
    ...
    // Create layers with group associations
    await Promise.all(
      defaultLayers.map(async (layer) => {
        // Find the group ID if this layer belongs to a group
        let groupId = null
        if (layer.groupId) {
          const group = groups.find((g) => g.name === layer.groupId)
          if (group) {
            groupId = group.id
          }
        }
