...
describe('isochrone proxy', () => {
  ...
  test('forwards the origin, mode and durations upstream', async () => {
    ...
    const res = await req(app).get('/proxy/isochrone', {
  ...
  describe('local validation — no upstream round trip', () => {
    const rejected: Array<[string, Record<string, string>]> = [
      ...
      // Barrelman accepts aliases; the proxy deliberately does not.
    ...
    for (const [label, query] of rejected) {
      test(`400s on ${label}`, async () => {
        const res = await req(app).get('/proxy/isochrone', { query })
  ...
  test('modes is cached for an hour and needs no origin', async () => {
    fetchResponses = [jsonResponse({ modes: [] })]

    const res = await req(app).get('/proxy/isochrone/modes')
  ...
  test('modes rejects an unauthenticated caller', async () => {
    setAuthUser(null)
