...
* The sun falls on the 3D buildings. They cast shadows across the streets, darken into the ground where a wall meets it, and shade by which way each wall faces — so a block reads as a row of separate buildings with depth between them instead of one grey mass. At night the shadows all but disappear, since there is no sun up, but the shading that separates one building from the next stays
