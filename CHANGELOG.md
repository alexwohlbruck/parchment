...
* Buildings carry more of their own recorded paint colour again in daylight, back to the strength it was tuned at before it was cut — the roof they tint is no longer near-white, so there is room for the hue without a street of brick reading as a wash over the land. A building's paint now darkens it as well as tinting it, which is what makes a brown building read as brown rather than tan: brown is dark orange, and a tint that carried only the hue drew a street of brick as gold. At night the tint is quieter than it has ever been: against dark ground a strong one reads as lit surfaces rather than painted ones, so a brick block is now a shade off a rendered one rather than its own colour
