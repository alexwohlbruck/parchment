...
export function loadCategories(
  ...
  console.log(`Loading OSM categories for language: ${language}`)
  ...
  try {
    ...
    for (const [presetId, rawPreset] of Object.entries(rawPresets)) {
      ...
      const category: CategoryResult = {
        ...
        type: 'category',
        name: localizedName,
        description: getPresetDescription(preset, presetTranslations[presetId]),
    ...
    console.log(
      `✅ Loaded ${categories.length} categories for ${language} in ${
        Date.now() - start
      }ms`,
    )
    return categories
 ...
 */
export function clearCategoryCache(language?: SupportedLanguage): void {
  if (language) {
